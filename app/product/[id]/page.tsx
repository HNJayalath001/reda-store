...
interface Product {
  ...
  description: string; sellingPrice: number; stockQty: number;
...
function Img({ src, alt, size = 18 }: { src: string; alt: string; size?: number }) {
  return (
...
export default function ProductPage() {
  ...
  const shareLink = typeof window !== "undefined" ? window.location.href : `https://redastore.lk/product/${id}`;
      ...
      <header className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-30">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/images/logo.png" alt={siteName} width={36} height={36}
          ...
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="p-6 bg-gray-50 border-b md:border-b-0 md:border-r border-gray-100">
              ...
              {ytId && (
                <div className="flex gap-2 mb-3">
                  <button onClick={()=>setShowVideo(false)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg font-semibold border-2 transition-colors ${!showVideo?"bg-blue-600 text-white border-blue-600":"bg-white text-gray-600 border-gray-200 hover:border-blue-300"}`}>
                    <Img src="/images/photos.svg" alt="Photos" size={14} />
                    Photos ({product.imageFileIds.length})
                  </button>
                  <button onClick={()=>setShowVideo(true)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg font-semibold border-2 transition-colors ${showVideo?"bg-red-600 text-white border-red-600":"bg-white text-gray-600 border-gray-200 hover:border-red-300"}`}>
                ...
                <>
                  <div className="w-full h-72 rounded-xl overflow-hidden bg-white flex items-center justify-center">
                    {product.imageFileIds.length > 0 ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={`/api/images/${product.imageFileIds[imgIdx]}`} alt={product.name}
                  ...
                  {product.imageFileIds.length > 1 && (
                    <div className="flex gap-2 mt-3 justify-center flex-wrap">
                      {product.imageFileIds.map((fid, i) => (
                        <button key={fid} onClick={() => setImgIdx(i)}
                          className={`w-14 h-14 rounded-lg overflow-hidden border-2 transition-colors ${i === imgIdx ? "border-blue-600" : "border-gray-200 hover:border-blue-300"}`}>
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={`/api/images/${fid}`} alt="" className="w-full h-full object-cover" />
                        </button>
                      ))}
                    </div>
                  )}
              ...
              <div className="mt-4 p-3 bg-white rounded-xl border border-gray-200">
                <p className="text-xs text-gray-500 mb-1.5 font-medium">🔗 Share this product</p>
                <div className="flex gap-2">
                  <input readOnly value={shareLink} className="flex-1 text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg px-2 py-1.5 truncate" />
                  <button onClick={copyLink}
                    className={`shrink-0 flex items-center gap-1 text-xs font-bold px-3 py-1.5 rounded-lg border-2 transition-colors ${copied?"bg-green-500 text-white border-green-500":"bg-blue-600 hover:bg-blue-700 text-white border-blue-600"}`}>
            ...
            <div className="p-6 flex flex-col">
              <div className="flex flex-wrap gap-2 mb-3">
                <span className="text-xs bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full font-medium">{product.category}</span>
                <span className="text-xs bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full">{product.brand}</span>
                {isOOS
                  ? <span className="text-xs bg-red-100 text-red-600 px-2.5 py-1 rounded-full font-semibold">Out of Stock</span>
                  : product.stockQty <= 5
                  ? <span className="text-xs bg-orange-100 text-orange-600 px-2.5 py-1 rounded-full font-semibold">Only {product.stockQty} left</span>
                  : <span className="text-xs bg-green-100 text-green-700 px-2.5 py-1 rounded-full font-semibold">In Stock</span>
                }
                {ytId && <span className="text-xs bg-red-100 text-red-600 px-2.5 py-1 rounded-full">▶ Video</span>}
              </div>

              <h1 className="text-2xl font-bold text-gray-900 mb-2">{product.name}</h1>
              <p className="text-3xl font-bold text-blue-600 mb-4">{formatRs(product.sellingPrice)}</p>
              ...
              <div className="mt-auto space-y-3">
                {!isOOS ? (
                  ...
                  <a href="/out-of-stock"
                    ...
                    <Img src="/images/requestorder.svg" alt="Request" size={18} /> Request When Available
                  </a>
                )}
